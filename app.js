const NoteApp = (() => {
  ...
  function setupSidebarToggles() {
    ...
    document.getElementById('pagesBtn')?.addEventListener('click', (e) => {
      e.stopPropagation();
      document.body.classList.toggle('pages-hidden');
      updateBackdrop();
      if (!document.body.classList.contains('pages-hidden')) {
        Editor.refreshPagesSidebar();
      }
    });
    ...
    document.getElementById('closePagesBtn')?.addEventListener('click', () => {
  ...
  function updateBackdrop() {
    ...
    const showBackdrop = !document.body.classList.contains('sidebar-hidden') ||
                         ...
                         !document.body.classList.contains('pages-hidden');
  ...
  function renderNoteList(notes) {
    ...
    notes.forEach(note => {
      ...
      const totalStrokes = (note.pages || []).reduce((sum, p) => sum + (p.strokes?.length || 0), 0);
      ...
      item.innerHTML = `
        ...
        <div class="note-item-preview">${totalStrokes} strokes across ${pageCount} page${pageCount > 1 ? 's' : ''}</div>
        <div class="note-item-meta">
          <span>${timeAgo}</span>
          ${pageCount > 1 ? `<span class="note-badge">📖 ${pageCount}p</span>` : ''}
  ...
  function showNoteContextMenu(e, note) {
    ...
    const actions = [
      { label: note.pinned ? '📌 Unpin' : '📌 Pin', action: async () => { note.pinned = !note.pinned; await DB.put(DB.STORES.NOTES, note); refreshNoteList(); } },
      { label: note.favorite ? '❤️ Unfavorite' : '❤️ Favorite', action: async () => { note.favorite = !note.favorite; await DB.put(DB.STORES.NOTES, note); refreshNoteList(); } },
      { separator: true },
      { label: note.trashed ? '♻️ Restore' : '🗑 Trash', action: async () => { note.trashed = !note.trashed; await DB.put(DB.STORES.NOTES, note); refreshNoteList(); }, danger: !note.trashed },
      ...(note.trashed ? [{ label: '💀 Delete Forever', action: async () => { if (!confirm('Delete permanently?')) return; await DB.remove(DB.STORES.NOTES, note.id); refreshNoteList(); }, danger: true }] : [])
    ];
    actions.forEach(a => {
      if (a.separator) { const sep = document.createElement('div'); sep.style.cssText = 'height:1px;background:var(--border);margin:4px 0;'; menu.appendChild(sep); return; }
  ...
  async function refreshFolders() {
    ...
    for (const folder of allFolders) {
      ...
      li.innerHTML = `<span>${folder.icon}</span><span class="folder-name">${folder.name}</span><span class="folder-count">${count}</span><button class="delete-folder">✕</button>`;
      ...
      li.querySelector('.delete-folder').addEventListener('click', async e => {
        ...
        if (currentFolderId === folder.id) { currentFolderId = null; currentFilter = 'all'; document.querySelector('[data-filter="all"]').classList.add('active'); }
  ...
  async function refreshTags() {
    ...
    allTags.forEach(tag => {
      ...
      pill.innerHTML = `#${tag.name} <button class="remove-tag">✕</button>`;
      ...
      pill.querySelector('.remove-tag').addEventListener('click', async e => {
        ...
        if (currentTagId === tag.id) { currentTagId = null; currentFilter = 'all'; document.querySelector('[data-filter="all"]').classList.add('active'); }
