// ===== CANVAS MODULE - APPLE PENCIL FIX + AUTO-HIDE MENU =====
const Canvas = (() => {
  ...
  function init() {
    ...
    // When drawing on canvas, hide menu immediately
    drawingCanvas.addEventListener('pointerdown', () => hidePenSettings(), { passive: true });
    ...
    console.log('✅ Canvas initialized. Current tool:', currentTool);
  }
  ...
  function setupAutoHide() {
    ...
    document.addEventListener('pointerdown', (e) => {
      ...
      // Check if click is inside pen settings or on a pen tool
      const clickedOnPanel = panel.contains(e.target);
      const clickedOnPenTool = e.target.closest('.pen-tool');
      const clickedOnFloatingToolbar = e.target.closest('.floating-toolbar');
      
      if (!clickedOnPanel && !clickedOnPenTool && !clickedOnFloatingToolbar) {
        hidePenSettings();
      }
    });
  }
  ...
  function resetAutoHideTimer() {
    ...
    clearTimeout(hideMenuTimer);
    hideMenuTimer = setTimeout(hidePenSettings, 4000);
  }

  function setupToolbar() {
    // ===== PEN PRESET BUTTONS (colored dots) =====
    document.querySelectorAll('.pen-preset').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        
        // If already active pen preset, TOGGLE menu
        ...
        // If clicked again on active pen, toggle menu
        if (wasActive) {
          togglePenSettings();
        } else {
          showPenSettings();
    ...
    // ===== OTHER TOOLS (eraser, lasso, highlighter, etc) =====
    ...
    // ===== COLOR PICKER (Auto-hide after change) =====
    const colorPicker = document.getElementById('penColorPicker');
    if (colorPicker) {
      ...
      colorPicker.addEventListener('change', e => {
        penColor = e.target.value;
        // Hide after user picks a color
        setTimeout(hidePenSettings, 300);
      });
    }

    // ===== SIZE SLIDER (Auto-hide after change) =====
    ...
    if (sizeSlider) {
      ...
      sizeSlider.addEventListener('change', () => {
        setTimeout(hidePenSettings, 800);
      });
    }

    // ===== OPACITY SLIDER (Auto-hide after change) =====
    const opacitySlider = document.getElementById('penOpacity');
    if (opacitySlider) {
      opacitySlider.addEventListener('input', e => { 
        penOpacity = parseFloat(e.target.value);
        resetAutoHideTimer();
      });
    ...
    // ===== KEEP MENU OPEN WHEN HOVERING/INTERACTING =====
    const penSettings = document.getElementById('penSettings');
    if (penSettings) {
      penSettings.addEventListener('pointerdown', e => {
        e.stopPropagation();
        resetAutoHideTimer();
      });
      penSettings.addEventListener('pointermove', () => resetAutoHideTimer());
      penSettings.addEventListener('mouseenter', () => clearTimeout(hideMenuTimer));
      penSettings.addEventListener('mouseleave', () => resetAutoHideTimer());
    }
  }
  ...
  function onPointerUp(e) {
    ...
    try {
      if (e && e.pointerId) drawingCanvas.releasePointerCapture(e.pointerId);
    } catch (err) {}
    ...
    if (currentStroke && currentStroke.points.length > 0) {
      ...
      if (shapeRecognitionEnabled && currentTool === 'pen' && currentStroke.points.length > 8) {
        ...
        if (shape) {
          ...
          if (typeof UI !== 'undefined') UI.showToast(`✨ Detected: ${shape.type}`);
        }
      }
  ...
  function detectShape(points) {
    ...
    if (isClosed) {
      ...
      const avgRadius = (width / 2 + height / 2) / 2;
    ...
    if (!isClosed && points.length > 10) {
      ...
      if (straightness > 0.85) return { type: 'Arrow', start: first, end: last };
    }
    return null;
  }
  ...
  function isStraightLine(points) {
    ...
    return calculateStraightness(points) > 0.95;
  }
  ...
  function clearCanvas() {
    ...
    redoStack = [];
    redrawStrokes();
    if (typeof UI !== 'undefined') UI.showToast('Canvas cleared');
  }

  function redrawStrokes() {
    if (!ctx) return;
    const w = drawingCanvas.width / (window.devicePixelRatio || 1);
    const h = drawingCanvas.height / (window.devicePixelRatio || 1);
