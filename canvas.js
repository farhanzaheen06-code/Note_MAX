// ===== NoteMax Canvas - RELIABLE WRITING =====
const Canvas = (() => {
  ...
  function showHoldIndicator(pos, type, duration) {
    ...
    if (!indicator) {
      ...
      indicator.id = id;
      indicator.style.cssText = `position:absolute;width:44px;height:44px;border:3px solid ${color};border-radius:50%;pointer-events:none;z-index:100;border-top-color:transparent;`;
  ...
  function showShapeSnapEffect(shape) {
    ...
    flash.style.cssText = `position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);padding:14px 28px;background:rgba(255,149,0,0.95);color:white;font-size:22px;font-weight:800;border-radius:24px;z-index:200;pointer-events:none;animation:shapeFlash 0.9s ease-out forwards;`;
  ...
  // ========== POINTER EVENTS - RELIABLE DRAWING ==========
  function onPointerDown(e) {
    ...
    // Draw initial dot for visibility
  ...
  function onPointerMove(e) {
    ...
    // Only add point if it's far enough from last (prevents duplicate points)
    ...
    if (scribbleEraseEnabled && currentTool !== 'eraser' && currentTool !== 'highlighter') {
      if (!scribbleDetected && currentStroke.points.length > 25) {
        ...
        const sdx = pos.x - scribbleHoldPos.x;
        const sdy = pos.y - scribbleHoldPos.y;
        const scribbleDistance = Math.sqrt(sdx * sdx + sdy * sdy);
        
        if (scribbleDistance > 30) {
    ...
    if (!scribbleDetected && currentTool === 'pen' && penStyle === 'solid' && 
      ...
      if (shapeHoldPosition) {
        const sdx = pos.x - shapeHoldPosition.x;
        const sdy = pos.y - shapeHoldPosition.y;
        if (Math.sqrt(sdx * sdx + sdy * sdy) > SHAPE_HOLD_MOVEMENT_THRESHOLD) {
    ...
    if (currentTool === 'eraser') {
      ...
      // RELIABLE DRAWING: Draw the segment directly
      drawLiveSegment(lastPoint, pos);
    }
  ...
  function onPointerUp(e) {
    ...
    if (currentStroke && currentStroke.points.length > 0) {
      ...
      // Final redraw to ensure everything is clean
      redrawStrokes();
    }
    currentStroke = null;
    shapeHoldTriggered = false;
  ...
  // ========== LIVE SEGMENT DRAWING (RELIABLE) ==========
  function drawLiveSegment(p1, p2) {
    if (!ctx || !currentStroke) return;
    
    const style = currentStroke.style || 'solid';
    
    // For fancy styles, do full redraw
    if (style !== 'solid') {
      redrawStrokes();
    ...
    // SOLID: Simple direct line drawing
    const isHighlighter = currentStroke.tool === 'highlighter';
    ctx.save();
    ctx.globalAlpha = isHighlighter ? 0.3 : currentStroke.opacity;
    ctx.globalCompositeOperation = 'source-over';
    ...
    const width = isHighlighter 
      ? currentStroke.size 
      : currentStroke.size * (0.7 + p2.pressure * 0.5);
    ctx.lineWidth = width;
  ...
  function renderSolid(stroke) {
    ...
    if (stroke.tool === 'pen' && !isHighlighter) {
      // Pressure-sensitive rendering with smooth curves
  ...
  function detectShape(points) {
    ...
    if (isClosed) {
      ...
      if (corners === 4) {
        ...
        points.forEach(p => {
          const nearEdge = 
            Math.abs(p.x - minX) < width * 0.12 || Math.abs(p.x - maxX) < width * 0.12 ||
            Math.abs(p.y - minY) < height * 0.12 || Math.abs(p.y - maxY) < height * 0.12;
  ...
  function detectStar(points, cx, cy) {
    ...
    if (peaks >= 4 && peaks <= 6) {
      return { type: 'Star', centerX: cx, centerY: cy, outerRadius: Math.max(...distances), innerRadius: Math.min(...distances), points: peaks };
    }
    return null;
  }
  ...
  function isStraightLine(points) {
    return points.length >= 3 && calculateStraightness(points) > 0.96;
  }
